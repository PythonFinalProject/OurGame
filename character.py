...
class Player(Character):
    def __init__(self, x, y, width, height):
        ...
        self.player_selection = {"1P": {"left": pygame.K_a, "right": pygame.K_d, "up": pygame.K_w, "down": pygame.K_s}, "2P": {"left": pygame.K_LEFT, "right": pygame.K_RIGHT, "up": pygame.K_UP, "down": pygame.K_DOWN}}

    def control(self, run, win_width, win_height, num_player):
        keys = pygame.key.get_pressed()
        player_control = self.player_selection[num_player]

        if keys[player_control["left"]] and self.x > 0:
            self.x -= self.vel
            self.left = True
            self.right = False
        elif keys[player_control["right"]] and self.x < win_width - self.width:
            self.x += self.vel
            self.right = True
            self.left = False
        if keys[player_control["up"]] and self.y > 0:
            self.y -= self.vel
            self.up = True
            self.down = False
        elif keys[player_control["down"]] and self.y < win_height - self.height:
            self.y += self.vel
            self.down = True
            self.up = False
        if keys[player_control["left"]] == 0 and keys[player_control["right"]] == 0 and keys[player_control["up"]] == 0 and keys[player_control["down"]] == 0:
            self.right = False
            self.left = False
            self.down = False
            self.up = False
...
class Enemy(Character):
    def __init__(self, x, y, width, height):
        ...
        self.vel = 3
        pygame.time.set_timer(CREATE_ENEMY_EVENT, 800) # Create enemy every 1 sec
